...
tools/proof-client/   Minimal status, smoke trigger, move-abs, and click-abs helper
...
- no arbitrary key API, repeated key path, arbitrary movement report, wheel report builder, or general button report builder;
...
- no IOCTL carrying HID report bytes, key codes, text, arbitrary payloads, repeat counts, or arbitrary commands;

...
- `New-VhidDevCert.cmd` / `New-VhidDevCert.ps1` create or reuse an exact-subject local dev/test code-signing certificate, export the public certificate, and import it into trusted LocalMachine stores when `-Apply` is supplied.
- `Install-VhidDev.cmd` / `Install-VhidDev.ps1` perform preflight checks, stage a clean package directory, regenerate the catalog there, sign the staged package with an existing test certificate, install/update from the staged INF, create the root devnode with DevCon when it is missing, and run `proof-client status`.
...
- `New-VhidDevCert.ps1`, `Install-VhidDev.ps1`, and `Uninstall-VhidDev.ps1` default to dry-run mode. Use `-Apply` only in an approved isolated dev/test VM after reviewing the logged plan.
- Logs are written under `C:\vhid-lab\logs` by default.
- Certificate helper exports public certificates under `C:\vhid-lab\certs` by default. It does not create production certificates or request Microsoft signing.
...
- If the root-enumerated device is missing after package install, `Install-VhidDev.ps1` uses `devcon.exe install` with the staged INF and `ROOT\WindowsVhidStackVirtualInput`; pass `-SkipDeviceCreate` only when intentionally validating package staging without device creation.
- These scripts do not enable TESTSIGNING, reboot, or run generated input tests.

## Typical Reviewed VM Flow

```powershell
.\tools\dev\New-VhidDevCert.cmd
.\tools\dev\New-VhidDevCert.cmd -CertificateSubject "Windows VHID Stack Test Certificate" -Apply
.\tools\dev\Install-VhidDev.cmd
.\tools\dev\Install-VhidDev.cmd -CertificateSubject "Windows VHID Stack Test Certificate" -Apply
